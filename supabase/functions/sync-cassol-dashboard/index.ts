// @ts-nocheck
  ...
  const corsHeaders = {
    ...
    "Access-Control-Allow-Headers": "authorization, apikey, content-type, x-client-info, x-cassol-dashboard-webhook, x-cassol-dashboard-session",
  };
  ...
  type DashboardProject = Record<string, unknown> & {
    ...
    empresa?: string;
    fim?: string;
    tarefas?: Array<{ id?: number | string; taskId?: number | string; checklistSyncId?: string; nome?: string; feito?: boolean; resp?: string; eventId?: number | string | null }>;
  };
...
const CASSOL_DASHBOARD_SYNC_VERSION = "3.6.1";
  ...
  const dashboardSessionBase64Url = (value: Uint8Array | string) => {
    const bytes = typeof value === "string" ? tokenEncoder.encode(value) : value;
    let binary = "";
    bytes.forEach(byte => binary += String.fromCharCode(byte));
    return btoa(binary).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  };
  async function signDashboardSession(payload: string, secret: string) {
    const key = await crypto.subtle.importKey("raw", tokenEncoder.encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    return dashboardSessionBase64Url(new Uint8Array(await crypto.subtle.sign("HMAC", key, tokenEncoder.encode(payload))));
  }
  async function createDashboardSession(user: string, secret: string) {
    const payload = dashboardSessionBase64Url(JSON.stringify({ user, exp: Date.now() + 12 * 60 * 60 * 1000 }));
    return `${payload}.${await signDashboardSession(payload, secret)}`;
  }
  async function verifyDashboardSession(token: string, secret: string) {
    const [payload, signature] = token.split(".");
    if (!payload || !signature || signature !== await signDashboardSession(payload, secret)) return null;
    try {
...
function isDashboardLinkedContext(context: Record<string, unknown>) {
  return normalize(context.source) === "cassol_dashboard"
    || context.cassol_dashboard_linked === true
    || Boolean(context.cassol_dashboard_event_id)
    || String(context.sync_token || "").startsWith("cassol-dashboard-");
}
...
async function protectRecentChecklistCompletion(
  ...
  const nextContext = {
    ...
    cassol_dashboard_completed: completion.completed,
    cassol_dashboard_last_observed_completed: completion.completed,
    cassol_dashboard_completion_date: completion.date,
  };
...
async function notifyMilenaWhenEditIsReady(task: ChecklistTask, actorId: string) {
  const milena = configuredDashboardRecipients().find(recipient => recipient.key === "milena");
  if (!milena?.userId) return false;
  ...
  const response = await fetch(`${supabaseUrl}/functions/v1/send-task-push`, {
    ...
    body: JSON.stringify({
      ...
      actor_id: actorId,
      event_type: "cassol-edit-completed",
      recipient_user_ids: [milena.userId],
      dashboard_push_title: "Conteúdo pronto para aprovação",
      dashboard_push_body: `Luiggi concluiu “${String(task.title || "Para editar").slice(0, 180)}”.`,
      dashboard_push_tag: `cassol-edit-completed-${task.id}-${Date.now()}`,
    }),
  });
  if (!response.ok) syncLog("push-milena-edicao-falhou", { taskId: String(task.id), status: response.status });
  return response.ok;
}
...
async function assertChecklistCompletion(
  ...
  async function confirmChecklistTaskDeletion(
    admin: ReturnType<typeof createClient>,
    taskId: string,
    operation: string,
  ) {
    if (operation !== "delete") return;
    const { error } = await admin.from("tasks").update({ is_active: false }).eq("id", taskId);
    if (error) throw error;
  }
  ...
  function projectTaskEvents(projects: DashboardProject[]) {
    return projects.flatMap(project => {
      ...
      return (project.tarefas || [])
        ...
        .map((task, taskIndex) => {
          const projectTaskKey = projectTaskStableKey(task);
          return {
          id: `project:${projectId}:${projectTaskKey}`,
          ...
          project_task_key: projectTaskKey,
        } as DashboardEvent;
        });
    });
  }

  // A ordem das subtarefas pode mudar no Dashboard. A chave usa um ID próprio
  // quando existe e, para registros antigos, uma assinatura estável do item.
  function projectTaskStableKey(task: { id?: unknown; taskId?: unknown; checklistSyncId?: unknown; nome?: unknown; resp?: unknown }) {
    const explicitId = String(task.id || task.taskId || task.checklistSyncId || "").trim();
    if (explicitId) return `id-${explicitId}`;
    const signature = `${normalize(task.nome)}|${normalize(task.resp)}`;
    let hash = 2166136261;
  ...
  async function deactivateTasksRemovedFromDashboard(
    ...
    const removedTaskIds = (activeTasks || []).filter(task => {
      ...
      const isDashboardLinked = isDashboardLinkedContext(context);
  ...
  // "Corrigir etapas do conteúdo" chegou a ser criada por engano como etapa
  // de conteúdo. Ela foi retirada do fluxo; esta limpeza pontual também a
  // remove dos Checklists que já a receberam, inclusive em sync parcial.
  async function deactivateRetiredContentStageTasks(
    admin: ReturnType<typeof createClient>,
    actorId: string,
  ) {
    const { data: retiredTasks, error: retiredTasksError } = await admin
      .from("tasks")
      .select("id")
      .eq("user_id", actorId)
      .eq("is_active", true)
      .contains("context", {
        cassol_dashboard_source: "content_stage",
        cassol_dashboard_stage_key: "corrigir",
      });
    if (retiredTasksError) throw retiredTasksError;
    const ids = (retiredTasks || []).map(task => String(task.id));
    if (!ids.length) return 0;
    const { error: deactivateError } = await admin.from("tasks").update({ is_active: false }).in("id", ids);
    if (deactivateError) throw deactivateError;
    const { error: completionsError } = await admin.from("completions").delete().in("task_id", ids);
    if (completionsError) throw completionsError;
    return ids.length;
  }

  async function deactivateLockedContentStageTasks(
    admin: ReturnType<typeof createClient>,
    actorId: string,
    contents: DashboardContent[],
  ) {
    const contentIds = new Set(contents.map(content => String(content.id || "")).filter(Boolean));
    if (!contentIds.size) return 0;
    const unlockedStageIds = new Set(contentStageEvents(contents).map(event => String(event.id)));
    ...
    const ids = (activeTasks || []).filter(task => {
      const context = parseTaskContext(task.context);
      if (context.cassol_dashboard_source !== "content_stage") return false;
      const contentId = String(context.cassol_dashboard_content_id || "");
      const stageKey = String(context.cassol_dashboard_stage_key || "");
      return contentIds.has(contentId) && !unlockedStageIds.has(`content:${contentId}:${stageKey}`);
    }).map(task => String(task.id));
    if (!ids.length) return 0;
    const { error: deactivateError } = await admin.from("tasks").update({ is_active: false }).in("id", ids);
    if (deactivateError) throw deactivateError;
  ...
  // Quando uma tarefa deixa de ser atribuída ao usuário, ela sai da lista
  // ativa desse Checklist. O vínculo permanece no contexto para permitir uma
  // futura reatribuição sem criar duplicatas.
  async function markTasksNoLongerAssignedToDashboardRecipient(
    ...
    for (const task of activeTasks || []) {
      ...
      const { error: updateError } = await admin.from("tasks")
        .update({ is_active: false, context: {
          ...context,
      ...
      const { error: completionError } = await admin.from("completions").delete().eq("task_id", task.id);
  ...
  async function sendDashboardAssignmentPushes(notices: Array<{ taskId: string; title?: string; body?: string }>, actorId: string) {
    const uniqueNotices = [...new Map(notices.filter(notice => notice.taskId).map(notice => [notice.taskId, notice])).values()];
    if (!uniqueNotices.length) return 0;
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
    let requested = 0;
    ...
    for (const notice of uniqueNotices) {
      ...
      try {
        const response = await fetch(`${supabaseUrl}/functions/v1/send-task-push`, {
          ...
          body: JSON.stringify({ task_id: taskId, actor_id: actorId, event_type: "dashboard-assigned", dashboard_push_title: notice.title, dashboard_push_body: notice.body }),
        });
        if (!response.ok) {
  ...
  async function updateDashboardContentStageFromChecklist(
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const changed = JSON.stringify(content.etapasStatus || {}) !== JSON.stringify(nextEtapasStatus);
      if (!changed) return { handled: true, changed: false };
  
      content.etapasStatus = nextEtapasStatus;
  ...
  function projectTaskLinkFromTask(task: ChecklistTask) {
    ...
    const taskKey = String(context.cassol_dashboard_project_task_key || "");
    return projectId && (taskKey || (Number.isInteger(taskIndex) && taskIndex >= 0)) ? { projectId, taskIndex, taskKey } : null;
  }
  ...
  async function updateDashboardProjectTaskFromChecklist(
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const currentIndex = link.taskKey
        ? (project?.tarefas || []).findIndex(projectTask => projectTaskStableKey(projectTask) === link.taskKey)
        : link.taskIndex;
      const currentTask = project?.tarefas?.[currentIndex];
      ...
      project.tarefas![currentIndex] = nextTask;
  ...
  async function importDashboardTasksToChecklist(
    ...
    serviceAccount: FirebaseServiceAccount,
    accessToken: string,
    directDocument?: { key: DashboardDocumentKey; value: unknown[] },
    ...
    const assignmentNotifications = new Map<string, { taskId: string; title?: string; body?: string }>();
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const [snapshot, contentsSnapshot, booksSnapshot, projectsSnapshot] = directDocument
        ? [
            { events: directDocument.key === "gc-events" ? directDocument.value as DashboardEvent[] : [], updateTime: "" },
            { contents: directDocument.key === "gc-conteudos" ? directDocument.value as DashboardContent[] : [], updateTime: "" },
            { books: directDocument.key === "gc-livros" ? directDocument.value as DashboardBook[] : [], updateTime: "" },
            { projects: directDocument.key === "gc-projetos" ? directDocument.value as DashboardProject[] : [], updateTime: "" },
          ]
        : await Promise.all([
      ...
      const retiredContentStages = await deactivateRetiredContentStageTasks(admin, actorId);
      const lockedContentStages = await deactivateLockedContentStageTasks(admin, actorId, contentsSnapshot.contents);
      ...
      const deleted = (directDocument ? 0 : await deactivateTasksRemovedFromDashboard(admin, actorId, allDashboardItems)) + retiredContentStages + lockedContentStages;
      ...
      for (const event of dashboardTasks) {
        ...
        // O ID de origem é suficiente para reconhecer importações antigas.
        // Versões anteriores nem sempre gravavam `source`, o que fazia essas
        // cópias escaparem da consolidação e reaparecerem para o Luiggi.
        const matchingLinkedTasks = linkedTasks || [];
        let matchingProjectTasks: typeof matchingLinkedTasks = [];
        if (event.dashboard_source === "project_task") {
          const projectId = String(event.project_id || "");
          const projectTaskKey = String(event.project_task_key || "");
          const { data: projectLinkedTasks, error: projectLinkedTasksError } = await admin
            .from("tasks")
            .select("id,title,category,category_id,user_id,created_at,is_active,is_recurring,context")
            .eq("user_id", actorId)
            .contains("context", { cassol_dashboard_project_id: projectId })
            .order("created_at", { ascending: true });
          if (projectLinkedTasksError) throw projectLinkedTasksError;
          matchingProjectTasks = (projectLinkedTasks || []).filter(candidate => {
            const candidateContext = parseTaskContext(candidate.context);
            if (candidateContext.cassol_dashboard_source !== "project_task") return false;
            if (String(candidateContext.cassol_dashboard_project_task_key || "") === projectTaskKey) return true;
            // Migra os vínculos antigos, que usavam apenas o índice da lista.
            return normalize(candidate.title) === normalize(title)
              && normalize(candidateContext.cassol_dashboard_last_observed_responsavel) === normalize(event.responsavel);
          });
        }
        const matchingTasks = [...new Map([...matchingLinkedTasks, ...matchingProjectTasks].map(candidate => [String(candidate.id), candidate])).values()];
        if (!task && matchingTasks.length) {
          task = matchingTasks.find(candidate => candidate.is_active === true) || matchingTasks[0];
        }
        if (task && matchingTasks.length > 1) {
        ...
        if (!task) {
          const context = {
            ...
            cassol_dashboard_project_task_key: event.dashboard_source === "project_task" ? String(event.project_task_key || "") : undefined,
          ...
          const nextContext = {
            ...
            cassol_dashboard_project_task_key: event.dashboard_source === "project_task" ? String(event.project_task_key || "") : currentContext.cassol_dashboard_project_task_key,
            cassol_dashboard_completion_date: dueDate,
            cassol_dashboard_completed: Boolean(event.arquivada),
            cassol_dashboard_assigned_to_recipient: true,
            cassol_dashboard_recipient: recipient.key,
        ...
        if (taskWasCreated || taskWasInactive || wasPreviouslyUnassigned) {
          ...
          const previousStage = String(event.content_previous_stage_name || "");
          if (isContentStage(event) && previousStage) {
            assignmentNotifications.set(taskId, {
              taskId,
              title: "Próxima etapa liberada",
              body: `A etapa “${previousStage}” de “${String(event.titulo || "Conteúdo").replace(/^\[[^\]]+\]\s*/, "")}” foi finalizada. Sua etapa está liberada.`,
            });
          } else {
            assignmentNotifications.set(taskId, { taskId });
          }
        }
      ...
      // Uma varredura completa pode ter começado antes do check no Checklist e
      // por isso respeita a janela de proteção. Já directDocument representa
      // uma ação recém-confirmada no Dashboard: ela é autoritativa e deve
      // conseguir marcar ou desmarcar imediatamente.
      if (!directDocument && await isChecklistCompletionProtected(admin, String(task.id))) {
        syncLog("conclusao-protegida", { taskId: String(task.id), eventId });
      } else {
        // Uma mudança de prazo não pode deixar uma conclusão antiga ligada à
        // tarefa. Mantemos somente o estado correspondente à data atual.
        const { error: staleCompletionError } = await admin.from("completions")
          .delete()
          .eq("task_id", task.id)
          .neq("date", dueDate);
        if (staleCompletionError) throw staleCompletionError;
        if (event.arquivada) {
    ...
    const suppressOwnActionNotifications = Boolean(notificationActorKey) && normalize(notificationActorKey) === normalize(recipient.key);
    if (!dashboardChanged || directDocument) {
      const assignment_pushes_requested = suppressOwnActionNotifications ? 0 : await sendDashboardAssignmentPushes([...assignmentNotifications.values()], actorId);
      const update_pushes_requested = suppressOwnActionNotifications ? 0 : await sendDashboardUpdatePushes(
  ...
  Deno.serve(async request => {
    ...
    try {
      ...
      const dashboardLogin = String(input.operation || "") === "dashboard_login";
      const dashboardSessionPull = String(input.operation || "") === "dashboard_session_pull";
      const dashboardSessionSecret = String(Deno.env.get("CASSOL_DASHBOARD_SESSION_SECRET") || "");
      if (dashboardLogin) {
      ...
      } else if (dashboardSessionPull) {
        if (!dashboardSessionSecret) return json({ error: "Sessão do Dashboard não configurada." }, 503);
        const dashboardSession = await verifyDashboardSession(String(request.headers.get("x-cassol-dashboard-session") || ""), dashboardSessionSecret);
        if (!dashboardSession) return json({ error: "Sessão do Dashboard inválida ou expirada." }, 401);
        dashboardSessionUser = String(dashboardSession.user || "");
      } else {
      ...
      if (!dashboardWebhookCall && !dashboardSessionPull && !allRecipientsPull && !actorId) return json({ error: "Usuário da sincronização ausente." }, 401);

      const recipient = actorId ? dashboardRecipientForUserId(actorId) : null;
      if (!dashboardWebhookCall && !dashboardSessionPull && !allRecipientsPull && !recipient) {
      ...
      const operation = requestedOperation === "pull" || requestedOperation === "pull_all" || requestedOperation === "dashboard_webhook" || requestedOperation === "dashboard_session_pull"
        ? "pull"
      ...
      if (operation === "pull") {
        const serviceAccount = parseFirebaseServiceAccount();
        const accessToken = await getFirebaseAccessToken(serviceAccount);
        if (dashboardWebhookCall || dashboardSessionPull || allRecipientsPull) {
          const results: Record<string, unknown> = {};
          const requestedRecipients = Array.isArray(input.recipients)
            ? new Set(input.recipients.map((item: unknown) => normalize(item)))
            : new Set<string>();
          // O gatilho do Firebase informa quais responsáveis realmente foram
          // afetados. Quando a lista vier vazia (formato antigo ou alteração que
          // não pôde ser identificada), a varredura completa continua sendo o
          // fallback seguro.
          const recipientsToPull = (dashboardWebhookCall || dashboardSessionPull) && requestedRecipients.size > 0
            ? configuredRecipients.filter(item => requestedRecipients.has(normalize(item.key)))
            : configuredRecipients;
          const directKey = ["gc-events", "gc-conteudos", "gc-livros", "gc-projetos"].includes(String(input.source_document || ""))
            ? String(input.source_document) as DashboardDocumentKey
            : null;
          const directDocument = dashboardSessionPull && directKey && Array.isArray(input.document_value)
            ? { key: directKey, value: input.document_value as unknown[] }
            : undefined;
          await Promise.all(recipientsToPull.map(async webhookRecipient => {
            const pullSource = dashboardSessionPull ? "dashboard_session" : dashboardWebhookCall ? "firebase_webhook" : "scheduled_full_pull";
            ...
            const result = await importDashboardTasksToChecklist(
              ...
              accessToken,
              directDocument,
              dashboardSessionPull ? dashboardSessionUser : "",
            );
      ...
      const previousTaskContext = parseTaskContext(task.context);
      const shouldNotifyMilena = Boolean(
        completion?.completed
        && previousTaskContext.cassol_dashboard_completed !== true
        && normalize(task.category) === "cassol"
        && normalize(task.title).includes("para editar")
      );
      let syncTask = task as ChecklistTask;
      if (completion) syncTask = await protectRecentChecklistCompletion(admin, syncTask, completion);
      const serviceAccount = parseFirebaseServiceAccount();
      const accessToken = await getFirebaseAccessToken(serviceAccount);
      ...
      if (contentStageSync.handled) {
        ...
        await confirmChecklistTaskDeletion(admin, taskId, operation);
        if (shouldNotifyMilena) await notifyMilenaWhenEditIsReady(syncTask, actorId);
